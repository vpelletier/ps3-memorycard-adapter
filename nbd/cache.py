...
class FileDictCache(object):
  ...
  def __init__(self, filename, read_only=True):
    ...
    while True:
      ...
      data_len = readLength()
      if data_len is None:
        raise ValueError, 'Short read when expecting data length, record ' \
          '%i corrupted at %i' % (record_start, tell() - record_start)
      data = []
      append = data.append
      for x in xrange(data_len):
        item = readData()
        if item is None:
          raise ValueError, 'Short read when expecting data item, record ' \
            '%i corrupted at %i' % (record_start, tell() - record_start)
        append(item)
  ...
  def __setitem__(self, key, value):
    if not isinstance(value, tuple):
      raise TypeError, 'Value must be a tuple: %r (%s)' % (value, type(value))
    for item in value:
      if not isinstance(item, basestring):
        raise TypeError, 'Value elements must be strings: %r (%s) for value ' \
          '%r' % (item, type(item), value)
